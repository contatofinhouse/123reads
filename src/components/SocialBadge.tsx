...
export const SocialBadge: React.FC<SocialBadgeProps> = ({ platform, label, className = "" }) => {
  const isTikTok = platform === 'BookTok';
  
  return (
    <div 
      className={`social-badge ${isTikTok ? 'tiktok' : 'reddit'} ${className}`}
      style={{
        display: 'inline-flex',
        alignItems: 'center',
        gap: '4px',
        ...
        background: isTikTok ? 'rgba(0,0,0,0.05)' : 'rgba(255, 69, 0, 0.1)',
        color: isTikTok ? '#000' : '#FF4500',
        border: `1px solid ${isTikTok ? 'rgba(0,0,0,0.1)' : 'rgba(255, 69, 0, 0.2)'}`,
      }}
    >
      {isTikTok ? (
